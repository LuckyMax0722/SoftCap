...
class SoftGroup(nn.Module):
    ...
    def forward(self, batch, mode=''):
        if mode == 'train':
            return self.forward_train(batch, **batch)
        elif mode == 'val':
            return self.forward_scene_test(batch, **batch)
        elif mode == 'eval':
    ...
    @cuda_cast
    def forward_train(self, data_dict, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                      semantic_labels, instance_labels, instance_pointnum, instance_cls,
                      pt_offset_labels, spatial_shape, batch_size, instance_id, num_points, **kwargs):
        ...
        if not self.semantic_only:
            ...
            instance_loss, ious_on_cluster_mask, cls_labels = self.instance_loss(cls_scores, mask_scores, iou_scores,
                                                                                 proposals_idx,
            ...
            select_feats = torch.zeros([batch_size, feats.shape[1]]).float().cuda()
            ...
            for i in range(batch_size):
                ...
                end = torch.sum(proposal_each_scene[0:i + 1])
                if start != end:
                    object_feats[i, 0:proposal_each_scene[i], :] = feats[start:end, :]

            object_mask = torch.zeros(batch_size, self.grouping_cfg.max_num_proposal).bool().cuda()
            if cls_labels != None:
                proposal_mask = cls_labels <= self.instance_classes
                for i in range(batch_size):
                    start = torch.sum(proposal_each_scene[0:i])
                    end = torch.sum(proposal_each_scene[0:i + 1])
                    if start != end:
                        object_mask[i, 0:proposal_each_scene[i]] = proposal_mask[start:end]

            (loss, log_vars) = self.parse_losses(losses)

            data_dict["detection_loss"] = loss
            data_dict["detection_log_vars"] = log_vars
            data_dict["select_feats"] = select_feats
            data_dict["good_clu_masks"] = good_clu_masks
            data_dict["object_feats"] = object_feats
            data_dict["object_mask"] = object_mask
            data_dict["cls_scores"] = cls_scores
            data_dict["mask_scores"] = mask_scores
    ...
    def forward_scene_test(self, data_dict, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                           semantic_labels, instance_labels, instance_pointnum, instance_cls,
                           pt_offset_labels, spatial_shape, batch_size, num_points, **kwargs):
        losses = {}
    ...
    def forward_test(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        if not self.semantic_only:
            if 'instance' in self.test_cfg.eval_tasks or 'panoptic' in self.test_cfg.eval_tasks:
                ...
                proposals_idx, proposals_offset, proposal_each_scene = self.forward_grouping(semantic_scores,
                                                                                             pt_offsets,
                                                                                             batch_idxs,
                                                                                             coords_float,
                                                                                             num_points,
        ...
        return ret
        # return ret, cls_scores, iou_scores, mask_scores, proposals_idx
    ...
    def forward_grouping(self,
        ...
        for i in range(batch_size):
            ...
            for class_id in range(self.semantic_classes):
                ...
                proposals_idx[:, 1] = object_idxs[proposals_idx[:, 1].long()].int() + start_idx
        ...
        proposal_each_scene = torch.tensor(proposal_each_scene).long().cuda()
    ...
    def forward_instance(self, inst_feats, inst_map):
        ...
        return instance_batch_idxs, cls_scores, iou_scores, mask_scores, feats
