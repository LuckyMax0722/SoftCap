...
class ScannetDatasetConfig(object):
    ...
    def class2size(self, pred_cls, residual):
        ...
        return self.mean_size_arr[pred_cls-2] + residual

    def class2size_batch(self, pred_cls, residual):
        ''' Inverse function to size2class '''      
        return self.mean_size_arr[pred_cls-2] + residual

    def param2obb(self, center, size_class, size_residual):
    ...
    def param2obb_batch(self, center, size_class, size_residual):
        box_size = self.class2size_batch(size_class, size_residual)
        obb = np.zeros((center.shape[0], 6))
        obb[:, 0:3] = center
        obb[:, 3:6] = box_size
        return obb
